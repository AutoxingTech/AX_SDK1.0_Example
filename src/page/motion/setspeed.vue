  ...
  <div class="content_box">
    ...
    <div class="mode_box">
      ...
      <div class="map_banner div_center">
        ...
        <div class="mode_banner">
          <div class="btn_tools bg_btn" @click="changeSpeed();">{{$t('leftMenuSetSpeed')}}</div>
        </div>
      </div>
    </div>
    ...
    <Dialog
      ...
      <div class="speed_container">
        <div v-for="(item,i) in speedList" :key="i" class="speed_banner speed_btn" @click="doSelectSpeed(i);">
          {{item.desc}}
        </div>
      </div>
    </Dialog>
...
export default {
  ...
  data () {
    return {
      ...
      showDialog: false,
      speedList: [
        {desc: '设置0.2m/s', speed: 0.2},
        {desc: '设置0.3m/s', speed: 0.3},
        {desc: '设置0.5m/s', speed: 0.5},
        {desc: '设置0.6m/s', speed: 0.6},
        {desc: '设置0.8m/s', speed: 0.8},
        {desc: '设置1.0m/s', speed: 1.0},
        {desc: '设置1.2m/s', speed: 1.2}]
    }
  },
  mounted () {
  },
  methods: {
    ...
    async doSelectSpeed (index) {
      let speedObj = this.speedList[index]
      this.showLoading()
      let isOk = await this.axRobot.setSpeed(speedObj.speed)
      this.hideLoading()
      if (isOk === true) {
        this.showDialog = false
      }
