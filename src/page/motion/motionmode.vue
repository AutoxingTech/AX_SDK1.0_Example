    ...
    <div class="mode_box">
      ...
      <div class="map_banner div_center">
        ...
        <div class="mode_banner">
          <div class="btn_tools bg_btn" @click="changeMode(0);">{{$t('autoMode')}}</div>
          <div class="btn_tools bg_btn" @click="changeMode(1);">{{$t('manualMode')}}</div>
          <div class="btn_tools bg_btn" @click="setEmergency();">{{isEmergencyStop?$t('cancelEmergencyStop'):$t('emergencyStopMode')}}</div>
        </div>
      </div>
    </div>
...
import { AXRobot as AXRobotProd, AppMode as AppModeProd, MotionType, EmergencyType} from '@autoxing/robot-js-sdk'
// eslint-disable-next-line standard/object-curly-even-spacing
